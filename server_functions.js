...
function getRandomPin() {
    ...
    for (let i = 0; i < 4; i++) { // Iterating 4 times
...
function addPinEntry(email, pin) { // Parameters taken are the user email and the corresponding pin
    ...
    setTimeout(() => {
        ...
        let index = accountPins.findIndex(entry => entry.email === email && entry.pin === pin);
        // If entry is found
        if (index !== -1) {
